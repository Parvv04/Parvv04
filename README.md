<h1 align="center">Hi 👋, I'm Parvathi Pratap H</h1>
- Computer Science undergraduate at RRCE (2027).<br>
- Building web applications, automation tools, and software projects with Python, JavaScript, and React.<br>
- Projects: Gmail Meeting Notifier (Gmail API + NLP), Smart Health Monitoring System (OpenCV + MediaPipe).<br>
- International Services Director @ Rotaract | Open Source Contributor @ GSSoC 2025.<br> Always learning, building, and exploring new technologies.

## 🌐 Socials:
[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/parvathi-p-3707552a6) [![email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:parvathi.pratap.h@gmail.com) 

## 🚀 Featured Projects

### 📌 MailMinder

* Gmail meeting reminder system
* Built using Python, FastAPI, Electron, and Gmail API
* Uses NLP and regex to extract meeting details and send desktop notifications

### 📌 Smart Health Monitoring System

* Real-time health monitoring using computer vision
* Built with Python, OpenCV, MediaPipe
* Detects drowsiness, poor posture, and unhealthy screen-time habits

### 📌 Gesture Controlled Assistive Mobility & Communication System

* Sensor-based wearable glove for gesture recognition
* Uses ESP32, Flex Sensors, MPU6050, and Machine Learning
* Converts hand gestures into wheelchair controls and speech output

### 📌 Weather App

* Real-time weather forecasting application
* Built using JavaScript, REST APIs, and Fetch API
* Displays live weather data based on user-selected locations

### 📌 Expense Tracker

* Expense management application built with React
* Add, manage, and track expenses dynamically
* Uses React Hooks and state management for real-time updates


## 💻 Tech Stack:
### Languages
![Python](https://img.shields.io/badge/Python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![JavaScript](https://img.shields.io/badge/JavaScript-323330?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)

### Frontend
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![Responsive Design](https://img.shields.io/badge/Responsive_Design-4285F4?style=for-the-badge)
![DOM Manipulation](https://img.shields.io/badge/DOM_Manipulation-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Async APIs](https://img.shields.io/badge/Async_APIs-FF6B6B?style=for-the-badge)

### Backend & Data
![REST API](https://img.shields.io/badge/REST_API-009688?style=for-the-badge)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Python Scripting](https://img.shields.io/badge/Python_Scripting-3776AB?style=for-the-badge&logo=python&logoColor=white)

### AI / ML
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white)
![MediaPipe](https://img.shields.io/badge/MediaPipe-FF6F00?style=for-the-badge)
![Hugging Face](https://img.shields.io/badge/Hugging_Face-FFD21E?style=for-the-badge&logo=huggingface&logoColor=black)

### Tooling
![Git](https://img.shields.io/badge/Git-F05033?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-121011?style=for-the-badge&logo=github&logoColor=white)
![NPM](https://img.shields.io/badge/NPM-CB3837?style=for-the-badge&logo=npm&logoColor=white)
![Google Cloud](https://img.shields.io/badge/Google_Cloud-4285F4?style=for-the-badge&logo=googlecloud&logoColor=white)
![venv](https://img.shields.io/badge/venv-3776AB?style=for-the-badge&logo=python&logoColor=white)
![GitHub Pages](https://img.shields.io/badge/GitHub_Pages-222222?style=for-the-badge&logo=githubpages&logoColor=white)
## 📊 GitHub Stats:
![](https://nirzak-streak-stats.vercel.app/?user=Parvv04&theme=shadow_green&hide_border=false)<br/>
![](https://github-readme-stats.vercel.app/api/top-langs/?username=Parvv04&theme=shadow_green&hide_border=false&include_all_commits=true&count_private=true&layout=compact)

### Leetcode Status
![LeetCode Stats](https://leetcard.jacoblin.cool/parvv_04?theme=dark&font=Karma&ext=heatmap)

---
[![](https://visitcount.itsvg.in/api?id=Parvv04&icon=0&color=0)](https://visitcount.itsvg.in)

